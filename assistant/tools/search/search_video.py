...
class YouTubeSearchVideo(BaseModel):
    ...
    max_results: Optional[int] = Field(
        description="The number of videos to search for", default=10
    )
    ...
    relevance_language: Optional[str] = Field(
        description="The language code to use", default="en"
    )
...
class YouTubeSearchVideoTool(StructuredTool):
    ...
    def _run(
        ...
        search_results: list[Search] = advanced_video_search(
            query=query, channel_id=channel_id, max_results=max_results
        )
        search_results: list[dict] = [
            self._parse_search(search) for search in search_results
        ]
