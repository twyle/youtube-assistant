...
def add_video_to_playlist(video_title: str, playlist_title: str) -> PlaylistItem:
    ...
    resource_id: VideoResourceId = VideoResourceId(videoId=video_id)
...
def list_channel_playlists(channel_title: str) -> list[Playlist]:
    ...
    youtube_response: YouTubeResponse = youtube_client.find_channel_by_name(
        channel_title
    )
...
def list_playlist_videos(
    channel_title: str, playlist_title: str, max_results: Optional[int] = 10
) -> list[PlaylistItem]:
    ...
    playlist: str = process.extract(
        playlist_title, list(playlists.keys()), scorer=fuzz.ratio
    )[0][0]
